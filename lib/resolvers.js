...
module.exports = resolvers = {
    Query: {
        ...
        getPeople: async() => {  //Funcion para traer todos los cursos 
            let db, student = []
            db = await connectDb(); //Iniciamos conexion
            student = await db.collection('student').find().toArray(); //Traemos todos los elementos   
            return student;
        },
        getPerson: async (root, {id}) => {  //De esta forma recibimos los parametros de la busqueda //Funcion para traer por id 
            let db, student
            db = await connectDb();
    ...
    Person: { //De esta forma validamos de que tipo son aquellos que implementan la interface 
...
//Las variables se declaran como "$id" junto a su tipo

//Con alias puedo realizar varias consultas y diferenciarlas con un alias y con fragment, sirve para llamar 
//varios campos predefinidos de una misma "tabla" 



//Control de errores 
//Throw new Error('Este es un ejemplo para el manejo de errores.');
